...
my $perc = Utilities::round($flags{"raw"}{"counts"}{"cis"}*100/$flags{"raw"}{"counts"}{"total"},2);
print LOGS "Total \"cis\" read-pairs in the file\t".$flags{"raw"}{"counts"}{"cis"}." (".$perc."%)\n";
$perc = Utilities::round($flags{"raw"}{"counts"}{"trans"}*100/$flags{"raw"}{"counts"}{"total"},2);
print LOGS "Total \"trans\" read-pairs in the file\t".$flags{"raw"}{"counts"}{"trans"}." (".$perc."%)\n";
$perc = Utilities::round($flags{"raw"}{"counts"}{"linear_rescued"}*100/$flags{"raw"}{"counts"}{"total"},2);
print LOGS "Linearly Mapped and rescued read-pairs (UU/UR/RU)\t".$flags{"raw"}{"counts"}{"linear_rescued"}." (".$perc."%)\n";
$perc = Utilities::round($flags{"raw"}{"counts"}{"uninformative"}*100/$flags{"raw"}{"counts"}{"total"},2);
print LOGS "Uninformative read-pairs\t".$flags{"raw"}{"counts"}{"uninformative"}." (".$perc."%)\n";
$perc = Utilities::round($flags{"raw"}{"counts"}{"lowqual"}*100/$flags{"raw"}{"counts"}{"total"},2);
print LOGS "Low mapping quality read-pairs (MAPQ < 28)\t".$flags{"raw"}{"counts"}{"lowqual"}." (".$perc."%)\n";
$perc = Utilities::round($flags{"raw"}{"counts"}{"nn"}*100/$flags{"raw"}{"counts"}{"total"},4);
print LOGS "Read-pairs with at least 2 Ns in both the read-sequences\t".$flags{"raw"}{"counts"}{"nn"}." (".$perc."%)\n";
$perc = Utilities::round($flags{"raw"}{"counts"}{"outofindex"}*100/$flags{"raw"}{"counts"}{"total"},3);
print LOGS "Read-pairs mapping to non-selected chromosomes\t".$flags{"raw"}{"counts"}{"outofindex"}." (".$perc."%)\n";
$perc = Utilities::round($flags{"raw"}{"counts"}{"lonelymates"}*100/$flags{"raw"}{"counts"}{"total"},2);
print LOGS "Reads with only single mate mapped\t".$flags{"raw"}{"counts"}{"lonelymates"}." (".$perc."%)\n";
$perc = Utilities::round($flags{"raw"}{"counts"}{"wgs_nonchimera"}*100/$flags{"raw"}{"counts"}{"total"},2);
print LOGS "WGS non-chimeric reads\t".$flags{"raw"}{"counts"}{"wgs_nonchimera"}." (".$perc."%)\n";
$perc = Utilities::round($flags{"raw"}{"counts"}{"nonHicChimera"}*100/$flags{"raw"}{"counts"}{"total"},2);
print LOGS "Non-HiC chimeric read pairs\t".$flags{"raw"}{"counts"}{"nonHicChimera"}." (".$perc."%)\n";
$perc = Utilities::round($flags{"raw"}{"counts"}{"twojunctoins"}*100/$flags{"raw"}{"counts"}{"total"},3);
...
foreach my $cl (sort keys %{$flags{"reported"}{"class"}}){
    my $percent = Utilities::round($flags{"reported"}{"class"}{$cl}*100/$flags{"raw"}{"class"}{$cl},2);
    $percent = "(".$percent."%)";
...
foreach my $cl (sort keys %{$flags{"raw"}{"read1"}}){
    ...
    $p1 = "(".Utilities::round($flags{"reported"}{"read1"}{$cl}*100/$flags{"raw"}{"read1"}{$cl},2)."%)";
    $p2 = "(".Utilities::round($flags{"reported"}{"read2"}{$cl}*100/$flags{"raw"}{"read2"}{$cl},2)."%)";
...
my $percent = Utilities::round($ptotal*100/$flags{"raw"}{"counts"}{"total"},2);
$percent = "(".$percent."%)";
print LOGS "Total pairs used in the analyses:\t $ptotal $percent\n";
$percent = "(".Utilities::round($pt1*100/$pt2,2)."%)";
